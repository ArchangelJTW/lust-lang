...
impl JitCompiler {
    ...
    pub(super) fn compile_array_len(&mut self, dest: u8, array: u8) -> Result<()> {
        ...
        dynasm!(self.ops ; .arch riscv32i ; bltz a0, >fail ; mv t0, a0);
        self.store_t0_as_int(dest);
        Ok(())
    }
    ...
    pub(super) fn compile_get_field(
        ...
        if let Some(index) = field_index {
            ...
            let (name_ptr, name_len) = self.retain_string(field_name);
            let name_ptr = name_ptr as u32 as i32;
    ...
    pub(super) fn compile_call_method(
        ...
        let (name_ptr, name_len) = self.retain_string(method_name);
        let name_ptr = name_ptr as u32 as i32;
        let name_len = name_len as i32;
    ...
    pub(super) fn compile_new_struct(
        ...
        let (sname_ptr, sname_len) = self.retain_string(struct_name);
        let sname_ptr = sname_ptr as u32 as i32;
        let sname_len = sname_len as i32;
        let field_count = field_names.len() as i32;

        let mut fptrs: Vec<*const u8> = Vec::new();
        let mut flens: Vec<usize> = Vec::new();
        for n in field_names {
            let (ptr, len) = self.retain_string(n);
            fptrs.push(ptr);
            flens.push(len);
        }
        let fptrs_bits = self.retain_string_pointers(fptrs) as u32 as i32;
        let flens_bits = self.retain_string_lengths(flens) as u32 as i32;
    ...
    pub(super) fn compile_new_enum_unit(
        ...
        let (en_ptr, en_len) = self.retain_string(enum_name);
        let en_ptr = en_ptr as u32 as i32;
        let en_len = en_len as i32;
        let (vn_ptr, vn_len) = self.retain_string(variant_name);
        let vn_ptr = vn_ptr as u32 as i32;
        let vn_len = vn_len as i32;
